...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(1, self.inplanes, kernel_size=(1, 3), stride=(1, 1), padding='same', bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 64, layers[0])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=(1, 2), dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=(1, 2), dilate=replace_stride_with_dilation[1])
